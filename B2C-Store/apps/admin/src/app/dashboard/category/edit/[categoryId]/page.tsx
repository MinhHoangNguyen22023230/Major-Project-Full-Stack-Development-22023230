...
export default function EditCategory() {
    ...
    const { data, isLoading, isError } = trpc.crud.findCategoryById.useQuery({ id: categoryId || '' });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (selectedFile && uploadCategoryImage) {
            setUploading(true);
            try {
        ...
        try {
            ...
            setTimeout(() => router.push('/dashboard/category'), 1000);
        } catch (error) {
            let msg = '';
            if (error && typeof error === 'object' && 'message' in error && typeof (error as { message?: string }).message === 'string') {
                msg = (error as { message: string }).message;
            }
            setAlert({ message: msg || 'Failed to update category', type: 'error' });
        }
    };

    if (!categoryId) {
        return <div className='flex justify-center items-center text-center w-full min-h-screen'>Category ID not found</div>;
    }
    if (isLoading) {
    ...
    if (isError) {
        return <div className='flex justify-center items-center text-center w-full min-h-screen text-3xl text-red-500'>Error loading category</div>;
    }
    if (!data) {
        return <div className='flex justify-center items-center text-center w-full min-h-screen text-3xl text-yellow-500'>Category data not found</div>;
    }
        ...
        >
            <div className='main-section p-4'>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-md mx-auto mt-8">
                    ...
                    >
                        {updateCategory.isPending ? <Loader2 className="animate-spin w-5 h-5 inline-block" /> : 'Update Category'}
                    </button>
                </form>
