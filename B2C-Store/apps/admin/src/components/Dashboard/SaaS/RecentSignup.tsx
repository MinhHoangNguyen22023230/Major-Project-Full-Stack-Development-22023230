...
export default function RecentSignup() {
    ...
    return (
        <div className="main-section p-4 bg-[var(--navbar-and-sidebar-bg)] text-[var(--foreground)]">
            <h2 className="mb-4 font-bold text-[var(--card-title)]">Recent Signups</h2>
            {isLoading ? (
                    ...
                    <tbody>
                        {recent.map(u => (
                            <tr key={u.id} className="rounded shadow-sm bg-[var(--gallery)] text-[var(--foreground)]">
                                ...
                                <td className="px-2 py-1 text-xs">{u.createdAt ? new Date(u.createdAt).toLocaleDateString() : "-"}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
